...
public class UserServiceImpl implements UserService
{
    ...
    public User save(User user)
    {
        ...
        newUser.setPrimaryemail(user.getPrimaryemail()
            .toLowerCase());
        //clear old todos from newuser
        //for each task in user todos, create new todos, set desctiption, set completed , set to newuser
        newUser.getTodos().clear();
        for (Todos t: user.getTodos())
        {
            Todos newTodo = new Todos();
            newTodo.setDescription(t.getDescription());
            newTodo.setCompleted(t.getCompleted());
            newTodo.setUsers(newUser);
